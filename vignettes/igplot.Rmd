...
# add one edge
karate_2 = add_edges(
    karate, c("Mr Hi", "Actor 32", "Actor 13", "Actor 17")
)

set.seed(42)
dat_org = igplot(karate, layout = "layout_with_fr", return_data = TRUE)
set.seed(42)
dat_new = igplot(karate_2, layout = "layout_with_fr", return_data = TRUE)
```
Next, we apply the procrustes transform to the coordinates of `dat_new` with the coordinates of `dat_org` as the target. 
```{r}
dat_new_procrustes = procrustes(dat_new, dat_org)
igplot(dat_new_procrustes)
```
As the underlying graphs are different, the transform brings the plots not in perfect alignment. Still, it rotates and scales the plot, such that the "Mr Hi" and "John A" are clearly on the same side of the plot. 